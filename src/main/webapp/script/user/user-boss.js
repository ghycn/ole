...
function _generateTable() {
	var columns = [
		...
		{sTitle:i18n('user.salary'), mDataProp: 'baseSalary',sClass:'center position w90',mRender:function(data,type,full){
			if(data!=null){
				return parseFloat(data).toFixed(2);
			}
			return null;
		}},
		{sTitle:i18n('user.baseSalary'), mDataProp: 'salary',sClass:'center position baseSalary w90',mRender:function(data,type,full){
			if(data!=null){
				return parseFloat(data).toFixed(2);
			}
			return null;
