...
!function($) {
	...
	$.fn.validation.defaults = {
		validRules : [ {
			...
			name : 'password',
			validate : function(value) {
				return (!/^[\@A-Za-z0-9\!\#\$\%\^\&\*\.\~]{6,16}$/.test(value));// 可以全数字、可以全字母、 可以全特殊字符(~!@#$%^&*.)、 三种的组合、 可以是任意两种的组合、 长度（6~16位）
			},
			defaultMsg : '请输入6~16位的密码。'
		},{
			name : 'cellPhone',
			validate : function(value) {
				return (!/^[0-9]{11,20}$/.test(value));
			},
			defaultMsg : '请输入手机号码。'
		},{
			name : 'date',
			...
			name : 'length',
			validate : function(value) {
				return (!/^\S{0,50}$/.test(value));
			},
			defaultMsg : '不能超过50个字符'
		},{
			name : 'tel',
			validate : function(value) {
			...
			name : 'birthday',
			validate : function(value) {
				return (!/^(?:19\d{2}|200\d)-(?:1[0-2]|0?[0-9])-(?:3[01]|[12][0-9]|0?[0-9])$/.test(value));//生日验证范围为1900-2009
			},
			defaultMsg : '请输入生日。'
		}, {
			name : 'zipCode',
		...
		if (error) {
			if (!controlGroup.hasClass('error')) {
				if (errorEl.length > 0) {
					...
					controls.append('<span class="help-inline" style="color:red;">' + errorMsg + '</span>');
				}
